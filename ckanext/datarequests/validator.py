...
def validate_datarequest(context, request_data):
    ...
    avoid_existing_title_check = (
        context["avoid_existing_title_check"] if "avoid_existing_title_check" in context else False
    )
    ...
    # Check description
    if (
        datarequests.get_config_bool_value("ckan.datarequests.description_required", False)
    ...
    if len(request_data["description"]) > constants.DESCRIPTION_MAX_LENGTH:
        errors[tk._("Description")] = [
            tk._("Description must be a maximum of %d characters long") % constants.DESCRIPTION_MAX_LENGTH
        ]
